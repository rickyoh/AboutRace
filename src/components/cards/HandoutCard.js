...
export class HandoutCard extends React.Component {
  render() {
    const data = get(this, 'props.data');

    const { onOpen } = this.props
    const title = get(this, 'props.data.title')
    const link = get(this, 'props.data.field_link.uri')
    const field_hand = get(this, 'props.data.field_hand.processed')
      ...
      <Container onClick={ () => onOpen(link)} >
        <InnerContainer>
          <TopBlock>
            ...
            <Ticker>handout</Ticker>
          </TopBlock>
          <BottomBlock>
            <ThemeTitle>{title}</ThemeTitle>
            <Description>{field_hand}</Description>
          </BottomBlock>
        </InnerContainer>
      </Container>
    )
